...
import { ONE_MIN_MS, calcNextBoundaryMs, diffToMmSs, pad } from '../lib/roundUtils'

const ACCENT = '#ea580c'
const QUEUE_DURATION_S = 15
const SELECT_DURATION_S = 45
...
export default function QueueTestPage() {
  ...
  const [queueNumber] = useState(() => Math.floor(Math.random() * 800) + 100)
  ...
  useEffect(() => {
    const t = setInterval(() => {
      ...
      // standby: 버튼 활성화 여부만 갱신 (전이는 버튼 클릭으로만)

      if (phase === 'queue' && phaseEndMs.current > 0 && Date.now() >= phaseEndMs.current) {
        selectingStartMs.current = Date.now()
        phaseEndMs.current = Date.now() + SELECT_DURATION_S * 1000
        setPhase('selecting')
      }

      if (phase === 'selecting' && phaseEndMs.current > 0 && Date.now() >= phaseEndMs.current) {
  ...
  const nextQueue = diffToMmSs(calcNextBoundaryMs(ONE_MIN_MS))
  ...
  const currentQueueNo = (phase === 'queue' && phaseRemaining)
  ...
  const isUrgent = phaseRemaining !== null && phaseRemaining.minutes === 0 && phaseRemaining.seconds <= 10
  const scoreMsg = elapsedS
    ? (parseFloat(elapsedS) < 5 ? '매우 빠름' : parseFloat(elapsedS) < 15 ? '평균' : '연습이 필요해요')
    : ''
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fafafa', display: 'flex', flexDirection: 'column', color: '#111' }}>
      <header style={{ padding: '14px 24px', borderBottom: '1px solid #e5e5e5', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <span style={{ fontSize: '0.95rem', fontWeight: 600 }}>대기열 빠른 연습</span>
      </header>
        ...
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 24px', gap: '48px' }}>
          <div style={{ textAlign: 'center' }}>
            <p style={{
              ...
              {queueOpen ? '대기열 오픈 중' : '다음 대기열까지'}
            </p>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'clamp(4rem, 15vw, 6.5rem)', fontWeight: 700, lineHeight: 1, letterSpacing: '-0.04em', color: '#111' }}>
              00:{pad(nextQueue.seconds)}
            </div>
      ...
      {phase === 'queue' && phaseRemaining && (
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 24px', gap: '40px' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'clamp(3rem, 12vw, 5rem)', fontWeight: 700, color: '#111', lineHeight: 1 }}>
      ...
      {(phase === 'selecting' || phase === 'confirming') && phaseRemaining && (
        <main style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ width: '230px', borderLeft: '1px solid #e5e5e5', padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: '20px', flexShrink: 0 }}>
            <div>
              <p style={{ fontSize: '0.8rem', color: '#444', letterSpacing: '0.1em', margin: '0 0 6px', fontWeight: 600 }}>남은 시간</p>
              <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '2.4rem', fontWeight: 700, color: isUrgent ? ACCENT : '#111', margin: 0, lineHeight: 1, transition: 'color 0.3s' }}>
                {pad(phaseRemaining.minutes)}:{pad(phaseRemaining.seconds)}
              </p>
            </div>
            ...
            {phase === 'confirming' && selectedSeat && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                >예매 확정</button>
                <button onClick={() => { setSelectedSeat(null); setPhase('selecting') }}
      ...
      {phase === 'result' && (
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 24px', gap: '40px' }}>
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: '0.8rem', letterSpacing: '0.15em', fontWeight: 700, margin: '0 0 20px', color: !timedOut && selectedSeat ? '#16a34a' : '#888' }}>
              {!timedOut && selectedSeat ? 'BOOKED' : 'TIME OUT'}
            </p>
            {!timedOut && selectedSeat ? (
              <>
                <p style={{ fontSize: '1.2rem', fontWeight: 700, margin: '0 0 4px' }}>
                  {selectedSeat.section}구역 {selectedSeat.row}행 {selectedSeat.number}번
                </p>
                <p style={{ fontSize: '0.95rem', color: '#444', fontWeight: 500, margin: '0 0 32px' }}>{selectedSeat.price.toLocaleString()}원</p>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 'clamp(3rem, 12vw, 4.5rem)', fontWeight: 700, color: '#111', lineHeight: 1 }}>
                  {elapsedS}<span style={{ fontSize: '1.2rem', color: '#555', marginLeft: '4px' }}>s</span>
                </div>
                <p style={{ fontSize: '0.95rem', color: '#444', fontWeight: 500, marginTop: '10px' }}>{scoreMsg}</p>
              </>
            ) : (
              <p style={{ fontSize: '1rem', color: '#444', fontWeight: 500 }}>45초 내에 좌석을 선택하지 못했습니다.</p>
            )}
          </div>
